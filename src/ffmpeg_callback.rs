// File taken from https://github.com/zmwangx/rust-ffmpeg/pull/91
// ffmpeg_next has no way to set the logging callback as of 8.0.0

extern crate libc;
extern crate log as log_crate;
use ffmpeg_next::sys as sys;
...
fn convert_level(level: Level) -> LevelFilter
{
    match level {
...
impl Callback for LoggingCallback {
    fn call(context: &LogContext) {
        if let Some(log_level) = LevelFilter::from(convert_level(context.level())).to_level() {
